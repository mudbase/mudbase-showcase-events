...
of the two roles (organizer/attendee) must present a real session, so there
is no `ensure_anonymous_session` here. `require_session` below is this app's
equivalent of the reference web app's `<AuthGate>`.
...
async def require_session(request: Request) -> tuple[SessionUser, str] | None:
    ...
    page except /login and /register requires a real session (there is no
    anonymous/guest read — see module docstring). Returns `(user, token)` on
    success, or `None` if the visitor must be redirected to /login (no
    session, or a refresh attempt failed and the session was cleared)."""
