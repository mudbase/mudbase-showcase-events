...
import { bookingDocSchema, activityEntrySchema, type BookingDoc } from "@/api/schemas";
 ...
 *
 * Deliberately excludes "checked_in" bookings from the capacity count (see web/plan/build-plan.md
 ...
 * attendee's slot for someone else. Byte-for-byte port of web/src/lib/capacity.ts.
 */
export async function reconcileEventCapacity(eventId: string, capacity: number): Promise<void> {
  ...
  const live: BookingDoc[] = [...confirmedRes.data, ...waitlistedRes.data].sort(
  ...
  live.forEach((booking, index) => {
    ...
    if (shouldBeConfirmed && booking.status !== "confirmed") {
      corrections.push(
        mudbaseClient
          .updateDocument(bookingDocSchema, BOOKINGS_COLLECTION_ID, booking._id, { status: "confirmed" })
          .then(() =>
            mudbaseClient.createDocument(activityEntrySchema, ACTIVITY_COLLECTION_ID, {
              eventId,
      ...
      corrections.push(
        mudbaseClient
          .updateDocument(bookingDocSchema, BOOKINGS_COLLECTION_ID, booking._id, { status: "waitlisted" })
          .then(() =>
            mudbaseClient.createDocument(activityEntrySchema, ACTIVITY_COLLECTION_ID, {
              eventId,
              actorId: booking.userId,
