...
const ROLE_OPTIONS: { value: AppRole; label: string; hint: string }[] = [
  { value: "attendee", label: "Attendee", hint: "Browse events and book tickets" },
  { value: "organizer", label: "Organizer", hint: "Create events and check guests in" },
]
...
export function RegisterForm(): React.JSX.Element {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: { role: "attendee", agreedToTerms: false } })
    ...
    return (
      <div className="space-y-3 rounded-md border border-border bg-secondary/50 p-4 text-sm">
        ...
        <p className="text-muted-foreground">
          If email verification is required for this project, we sent a link to{" "}
          <span className="font-medium text-foreground">{submittedEmail}</span>. Otherwise you can sign in right away.
        </p>
      </div>
    )
  }
