...
/// results — `_id`/`createdAt`/`updatedAt` already typed, everything else in
/// `additionalProperties`) and a single document's raw `JSONValue` (from `DataResponse.data` on
/// create/get/update). Domain models (`EventItem`, `Booking`, `ActivityEntry`) build themselves from
/// this rather than each hand-rolling their own `DataListResponseDataInner`-vs-`JSONValue`
/// branching.
struct MudbaseDocument: Sendable {
    let id: String
    let createdAt: Date?
    let updatedAt: Date?
    ...
    /// `DataResponse.data` and `GetLocalSession200Response.user` are both untyped `JSONValue` — this
    /// reconstructs the same `{id, createdAt, updatedAt, ...fields}` shape from the raw dictionary,
    /// pulling out `_id`/`createdAt`/`updatedAt` the same way the generated
    ...
    /// Parses a scalar `string`-typed date field (e.g. `startsAt`), as opposed to `createdAt`/
    /// `updatedAt` which the SDK already types as `Date` on the list path.
    func date(_ key: String) -> Date? {
        string(key).flatMap(MudbaseDocument.parseDate(rawString:))
    }

    private static func parseDate(rawString: String) -> Date? {
        ISO8601DateFormatter.mudbase.date(from: rawString)
    }
}
