...
/// with no built-in message extraction. The platform's own error bodies are consistently shaped like
/// `{ "error": "..." }` or `{ "message": "..." }` (see `web/src/lib/mudbase.ts`'s hand-rolled client
/// for the same convention) — this decodes that shape when present and otherwise falls back to a
/// status-code-derived message so no failure ever surfaces a raw `Error` description to a user.
enum MudbaseAPIError {
    ...
    static func map(_ error: Error) -> DisplayableError {
        ...
        if statusCode == -1 {
            // Transport-level failure — offline, DNS failure, timeout, etc. `underlying` here is the
            // raw URLError/DecodingError the generator wraps rather than an HTTP status.
