...
module Mudbase
  ...
  # verbatim from the sibling `mudbase-showcase-kanban`/`mudbase-showcase-social` ports - see
  ...
  module ClientFactory
    # @param access_token [String, nil] Mudbase-issued JWT for one of this project's two roles
    #   (organizer/attendee). Omit for public endpoints (login).
    ...
    # the generator can't type) - every real field (`title`, `startsAt`, `capacity`, `status`,
    # ...) would be silently dropped by typed deserialization. `LoginLocalUser200ResponseUser`
    # also still doesn't declare `customRole`, which this app's RBAC gating depends on.
